...
import subprocess
def test_files():
    File1 = os.path.exists("./iac_bootstrap/infra_builder_terraform.py")
    File2 = os.path.exists("./iac_bootstrap/infra_bootstrap.py")
    File3 = os.path.exists("./iac_bootstrap/install_nginx.py")
...
def test_infra_bootstrap():
    ...
    test_provider = subprocess.Popen("python3 iac_bootstrap/infra_bootstrap.py --account mygroup-myenv --provider falseprovider", stderr=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
    out = test_provider.communicate()[1]
    assert out == b'Error in provider argument\n'

    test_action = subprocess.Popen("python3 iac_bootstrap/infra_bootstrap.py --account mygroup-myenv --action wrong", stderr=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
...
def test_infra_builder_terraform():
    ...
    test_provider = subprocess.Popen("python3 ../../iac_bootstrap/infra_builder_terraform.py --account mygroup-myenv --provider falseprovider", stderr=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
    out = test_provider.communicate()[1]
    assert out == b'Error in provider argument\n'

    test_action = subprocess.Popen("python3 ../../iac_bootstrap/infra_builder_terraform.py --account mygroup-myenv --action wrong", stderr=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
    out = test_action.communicate()[1]
    assert out == b"Error ,Bad Action for Terraform,just 'apply','plan' and 'destroy' are available\nYou must fix errors mentioned above\n"


    test_layers = subprocess.Popen("python3 ../../iac_bootstrap/infra_builder_terraform.py --account mygroup-myenv --layer wrong_layer", stderr=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
    out = test_layers.communicate()[1]
    assert out == b'Error in layer argument\n'

    test_ignore =  subprocess.Popen("python3 ../../iac_bootstrap/infra_builder_terraform.py --account mygroup-myenv --ignore wrong_layer", stderr=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
